...
ssp_interp <- ssp %>% 
  ...
  nest() %>% 
  mutate(total_pop_interp = map(data, ~approx(x = .$year, y = .$total_pop, xout = 2020:2100) %>% as_tibble())) %>% 
  ...
  left_join(fips_codes %>% select(state_abb = state, state = state_code, state_name))
...
poprocks <- bind_rows(past, fut) %>% 
  ...
  nest() %>% 
  mutate(total_pop_interp = map(data, ~approx(x = .$year, y = .$total_pop, xout = 1990:2100) %>% as_tibble())) %>% 
  unnest(total_pop_interp) %>% 
  rename(year = x, total_pop_interp = y) %>% 
  select(-data) %>% 
  mutate(race = recode(race, `1` = "White NH", `2` = "Black NH", `3` = "Hispanic", `4` = "Other NH")) %>% 
...
ssp_state <- poprocks %>% 
  ...
  summarize(state_race_pop = sum(total_pop_interp))

ssp_state %>% 
  ggplot(aes(x = year, y = state_race_pop, color = race, fill = race)) +
  geom_point() + geom_line() + facet_wrap(~state_name, scales = "free_y")
